...
function rememberParticipant(req,participant){
 if(!req.session)return;
 req.session.ludiAiContext={lastTopic:'participant_lookup',lastParticipantId:participant.id,lastParticipantName:`${participant.first_name} ${participant.last_name}`.trim(),lastGroupId:participant.groupid||participant.assignment_group_id||null,lastGroupName:participant.groupname||null,lastStandName:participant.stand_name||null,updatedAt:Date.now()};
}
function participantLabel(p){return `${p.first_name} ${p.last_name}`.trim();}
function participantGroupLabel(p){const group=p.stand_name?`${p.groupname||'Gruppo'} (${p.stand_name})`:p.groupname;return group||'nessuno stand/gruppo associato';}
function extractParticipantLookup(original){
 const m=String(original||'').trim().match(/^(?:cerca|trova|cerco|trovami|dati\s+di|informazioni\s+su|pass\s+di)\s+([a-zà-ù'\-]+(?:\s+[a-zà-ù'\-]+){0,3})[!?.\s]*$/i);
 return m?m[1].trim():null;
}
function isParticipantFollowUp(original){return /^(?:ha\s+un\s+pass|e\s+il\s+pass|e\s+il\s+suo\s+pass|e\s+stato\s+consegnato|è\s+stato\s+consegnato|e\s+invalidato|è\s+invalidato|di\s+quale\s+stand\s+fa\s+parte|in\s+quale\s+stand\s+(?:è|e)|dove\s+lavora|apri\s+(?:la\s+)?sua\s+scheda)[!?.\s]*$/i.test(String(original||'').trim());}
async function getParticipantById(id){
 if(!id)return null;
 return await new Promise(resolve=>db.get(`SELECT p.id,p.first_name,p.last_name,p.assignment_group_id,ag.id AS groupid,ag.name AS groupname,ag.stand_name FROM participants p LEFT JOIN assignment_groups ag ON ag.id=p.assignment_group_id WHERE p.id=?`,[id],(err,row)=>resolve(err?null:row)));
}
async function getParticipantPasses(participantId){return await new Promise(resolve=>db.all(`SELECT id,code,status,pdf_file,replaced_by,created_at FROM passes WHERE participant_id=? ORDER BY id DESC`,[participantId],(err,rows)=>resolve(err?[]:rows||[])));}
...
async function diagnoseParticipantLookup(req,original){
 const nameGuess=extractParticipantLookup(original);if(!nameGuess)return null;
 ...
 if(rows.length>1){const names=rows.slice(0,5).map(p=>`${participantLabel(p)} — ${participantGroupLabel(p)}`).join('; ');return out(`Ho trovato più nominativi: ${names}. Specifica nome e cognome completi.`,'/participants','Apri Assegnatari pass →','ricerca nominativo ambigua');}
 const p=rows[0];rememberParticipant(req,p);const passes=await getParticipantPasses(p.id);const active=passes.filter(x=>x.status!=='INVALIDATO');const passText=active.length?`Ha ${active.length} pass attivo${active.length===1?'':'i'}${active[0]&&active[0].status?`, stato ${active[0].status}`:''}.`:'Non ha pass attivi al momento.';
...
async function diagnoseParticipantFollowUp(req,original){
 const ctx=getLudiContext(req);if(!ctx||!ctx.lastParticipantId||!isParticipantFollowUp(original))return null;
 const p=await getParticipantById(ctx.lastParticipantId);if(!p)return null;const q=String(original||'').toLowerCase(),name=participantLabel(p),link=participantLink(p);
 if(/stand|dove\s+lavora/.test(q))return out(`${name} appartiene a ${participantGroupLabel(p)}.`,link,'Apri la scheda dello stand →','memoria conversazionale · nominativo');
 if(/apri.*scheda/.test(q))return out(`La scheda di ${name} è disponibile all’interno dello stand ${participantGroupLabel(p)}.`,link,'Apri la scheda dello stand →','memoria conversazionale · nominativo');
...
Puoi anche scrivere una domanda libera: se serve, cercherò nelle guide interne del portale.${pageHint}`,source:'Ludi AI · cosa posso fare',link:null,suggestions:allowed.slice(0,3).map(x=>x.replace(/[“”]/g,''))};
 ...
 app.post('/api/assistente/guida',requireAuth,async (req,res)=>{
  ...
  // 0bis) Ricerca esplicita di nominativo, anche con il solo nome.
  try{const participantLookup=await diagnoseParticipantLookup(req,original);if(participantLookup)return res.json(participantLookup);}catch(e){console.error('diagnoseParticipantLookup',e.message);}
