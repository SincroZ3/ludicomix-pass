...
db.run(`CREATE TABLE IF NOT EXISTS participants (
  ...
  email TEXT, role TEXT, stand_name TEXT, zone TEXT,
  ref_code TEXT, notes TEXT, assignment_group_id INTEGER, import_batch_id TEXT,
  FOREIGN KEY(assignment_group_id) REFERENCES assignment_groups(id)
)`);

db.run(`CREATE TABLE IF NOT EXISTS pass_types (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL, description TEXT, template_file TEXT NOT NULL,
  name_x INTEGER DEFAULT 100, name_y INTEGER DEFAULT 400,
  role_x INTEGER DEFAULT 100, role_y INTEGER DEFAULT 370,
  qr_color TEXT DEFAULT '#000000'
)`);

db.run(`CREATE TABLE IF NOT EXISTS passes (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  participant_id INTEGER NOT NULL, pass_type_id INTEGER NOT NULL,
  code TEXT, status TEXT DEFAULT 'GENERATO', pdf_file TEXT, replaced_by INTEGER,
...
db.run(`CREATE TABLE IF NOT EXISTS groups (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL, priority INTEGER DEFAULT 0, pass_type_id INTEGER,
  FOREIGN KEY(pass_type_id) REFERENCES pass_types(id)
)`);

db.run(`CREATE TABLE IF NOT EXISTS assignment_groups (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL, group_id INTEGER NOT NULL,
  stand_name TEXT, zone TEXT, stand_code TEXT, max_passes INTEGER,
  notes TEXT, email TEXT, portal_token TEXT,
  portal_enabled INTEGER DEFAULT 0, portal_open_from TEXT, portal_open_until TEXT,
  portal_status TEXT DEFAULT 'chiuso', contract_status TEXT DEFAULT 'bozza',
  map_row INTEGER, map_col INTEGER, map_span INTEGER DEFAULT 1,
  map_x REAL, map_y REAL, map_w REAL, map_h REAL, map_shape TEXT,
  max_auto_passes INTEGER DEFAULT 0, edition_id INTEGER,
...
db.run(`CREATE TABLE IF NOT EXISTS users (
  ...
  role TEXT NOT NULL DEFAULT 'operator', created_at TEXT DEFAULT (datetime('now'))
)`);
...
db.run(`CREATE TABLE IF NOT EXISTS pass_status_history (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  pass_id INTEGER NOT NULL, status TEXT NOT NULL,
  changed_at TEXT DEFAULT (datetime('now')), user_id INTEGER,
...
db.run(`CREATE TABLE IF NOT EXISTS zones (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL UNIQUE, sort_order INTEGER DEFAULT 0, background_image TEXT
)`);
...
db.run(`CREATE TABLE IF NOT EXISTS auto_passes (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  assignment_group_id INTEGER NOT NULL, code TEXT UNIQUE,
  status TEXT DEFAULT 'GENERATO', pdf_file TEXT, pass_number INTEGER, total_passes INTEGER,
  created_at TEXT DEFAULT (datetime('now')),
  FOREIGN KEY(assignment_group_id) REFERENCES assignment_groups(id)
)`);
...
db.run(`CREATE TABLE IF NOT EXISTS scan_attempts (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  code TEXT NOT NULL, result TEXT NOT NULL, pass_id INTEGER,
  participant_name TEXT, group_name TEXT, user_id INTEGER, ip TEXT,
  created_at TEXT DEFAULT (datetime('now'))
)`);
...
db.run(`CREATE TABLE IF NOT EXISTS announcements (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  title TEXT NOT NULL, message TEXT NOT NULL, emoji TEXT DEFAULT '📣',
  type TEXT DEFAULT 'info', is_pinned INTEGER DEFAULT 0,
  created_by INTEGER, created_at TEXT DEFAULT (datetime('now')), expires_at TEXT,
  FOREIGN KEY(created_by) REFERENCES users(id)
)`);

db.run(`CREATE TABLE IF NOT EXISTS announcement_reads (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  announcement_id INTEGER NOT NULL, portal_token TEXT NOT NULL,
...
db.run(`CREATE TABLE IF NOT EXISTS contacts (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  assignment_group_id INTEGER NOT NULL, name TEXT NOT NULL,
  role TEXT, email TEXT, phone TEXT, is_primary INTEGER DEFAULT 0,
  FOREIGN KEY(assignment_group_id) REFERENCES assignment_groups(id) ON DELETE CASCADE
)`);

db.run(`CREATE TABLE IF NOT EXISTS payments (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  assignment_group_id INTEGER NOT NULL, description TEXT,
  amount REAL NOT NULL, status TEXT DEFAULT 'da_pagare',
...
db.run(`CREATE TABLE IF NOT EXISTS group_documents (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  assignment_group_id INTEGER NOT NULL, filename TEXT NOT NULL,
  original_name TEXT, doc_type TEXT, notes TEXT, uploaded_by INTEGER,
...
db.run(`CREATE TABLE IF NOT EXISTS portal_documents (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  assignment_group_id INTEGER NOT NULL, doc_type TEXT NOT NULL,
  filename TEXT NOT NULL, original_name TEXT, status TEXT DEFAULT 'ricevuto',
  uploaded_at TEXT DEFAULT (datetime('now')), reviewed_by INTEGER,
...
db.run(`CREATE TABLE IF NOT EXISTS support_tickets (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  assignment_group_id INTEGER NOT NULL, portal_token TEXT NOT NULL,
  subject TEXT NOT NULL, message TEXT NOT NULL, status TEXT DEFAULT 'aperto',
...
db.run(`CREATE TABLE IF NOT EXISTS ticket_replies (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ticket_id INTEGER NOT NULL, message TEXT NOT NULL,
  is_admin INTEGER DEFAULT 0, author_name TEXT,
  created_at TEXT DEFAULT (datetime('now')),
...
db.run(`CREATE TABLE IF NOT EXISTS accreditation_requests (
  ...
  email TEXT NOT NULL, phone TEXT, stand_type TEXT, stand_size TEXT,
  notes TEXT, status TEXT DEFAULT 'in_attesa',
  reviewed_by INTEGER, reviewed_at TEXT, rejection_reason TEXT,
  assignment_group_id INTEGER, accreditation_type TEXT,
  media_outlet TEXT, press_role TEXT, publisher TEXT,
  genre TEXT, channel_url TEXT, platform TEXT, subscribers TEXT,
  created_at TEXT DEFAULT (datetime('now'))
)`);
...
db.run(`CREATE TABLE IF NOT EXISTS editions (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL, year INTEGER, is_current INTEGER DEFAULT 0,
  created_at TEXT DEFAULT (datetime('now'))
)`);
...
db.run(`CREATE TABLE IF NOT EXISTS spaces (
  ...
  capacity INTEGER NOT NULL DEFAULT 0, location TEXT,
  color TEXT DEFAULT '#4f98a3', active INTEGER NOT NULL DEFAULT 1,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
)`);

db.run(`CREATE TABLE IF NOT EXISTS speakers (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL, bio TEXT, photo_url TEXT,
  email TEXT, phone TEXT, social_url TEXT, notes TEXT,
...
db.run(`CREATE TABLE IF NOT EXISTS events (
  ...
  location_type TEXT NOT NULL DEFAULT 'space',
  space_id INTEGER REFERENCES spaces(id) ON DELETE RESTRICT,
  ...
  image_url TEXT, tags TEXT, notes TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
...
db.run(`CREATE TABLE IF NOT EXISTS event_speakers (
  ...
  role TEXT DEFAULT 'speaker', order_num INTEGER DEFAULT 0,
...
db.run(`CREATE TABLE IF NOT EXISTS registrations (
  ...
  notes TEXT, registered_at TEXT NOT NULL DEFAULT (datetime('now')),
  cancelled_at TEXT, UNIQUE(event_id, email)
)`);

// ── SCHEDE OSPITI PUBBLICHE (separate dagli speaker interni) ──
db.run(`CREATE TABLE IF NOT EXISTS guest_profiles (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL,
  category TEXT NOT NULL DEFAULT 'ospite',
  bio TEXT,
  photo_url TEXT,
  stand_location TEXT,
  website_url TEXT,
...
// ═══════════════════════════════════════════════════════
// MIGRAZIONI SICURE (no-op se colonne già presenti)
// ═══════════════════════════════════════════════════════

db.run(`ALTER TABLE events ADD COLUMN registrations_open INTEGER NOT NULL DEFAULT 0`, () => {});
db.run(`ALTER TABLE events ADD COLUMN location_type TEXT NOT NULL DEFAULT 'space'`, () => {});
db.run(`ALTER TABLE events ADD COLUMN custom_location TEXT`, () => {});
...
db.run(`CREATE INDEX IF NOT EXISTS idx_guest_profiles_pub ON guest_profiles(is_published, sort_order)`);
...
db.run(`DROP VIEW IF EXISTS v_public_program`, () => {
  ...
  SELECT
    ...
    e.event_type, e.image_url, e.tags, e.max_seats,
    e.registrations_open, e.featured,
    e.location_type, e.custom_location,
    CASE
      WHEN e.location_type = 'space' THEN COALESCE(s.name, 'Sala non assegnata')
      WHEN e.location_type = 'stand' THEN COALESCE(e.custom_location, 'Stand espositore')
      ELSE COALESCE(e.custom_location, 'Luogo esterno')
    END AS space_name,
    COALESCE(s.color, '#718096') AS space_color,
    COALESCE(s.capacity, 0) AS space_capacity,
...
].forEach(p => db.run('INSERT OR IGNORE INTO app_settings(key,value) VALUES(?,?)', p));
...
db.get('SELECT COUNT(*) AS n FROM editions', [], function(err, row) {
  if (!err && row && row.n === 0) {
    db.run("INSERT INTO editions (name, year, is_current) VALUES ('Ludicomix 2026', 2026, 1)", [], function(e2) {
      if (!e2) db.run('UPDATE assignment_groups SET edition_id = ? WHERE edition_id IS NULL', [this.lastID]);
    });
  } else {
    db.get('SELECT id FROM editions WHERE is_current=1 LIMIT 1', [], function(e3, ed) {
      if (!e3 && ed) db.run('UPDATE assignment_groups SET edition_id = ? WHERE edition_id IS NULL', [ed.id]);
    });
  }
});
